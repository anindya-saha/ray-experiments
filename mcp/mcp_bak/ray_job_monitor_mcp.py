#!/usr/bin/env python3
"""
MCP Server for Ray Job Monitoring

This server analyzes Ray job logs to provide insights about:
- Job execution status
- Resource allocation and pending demands
- Performance metrics
...
async def list_tools() -> List[Tool]:
    ...
    return [
        Tool(
            ...
            description="Analyze Ray job logs from console output to provide job status overview",
            inputSchema={
                "type": "object",
                "properties": {
                    "log_content": {
        ...
        Tool(
            name="get_job_status",
            description="Get current Ray job status summary including resource allocation and pipeline progress",
            inputSchema={
                "type": "object",
                "properties": {}
            }
        ...
        Tool(
            ...
            description="Check current resource allocation and any pending demands",
            inputSchema={
                "type": "object",
                "properties": {}
            }
        ...
        Tool(
            ...
            description="Get detailed progress of each pipeline stage",
            inputSchema={
                "type": "object",
                "properties": {}
            }
        ...
        Tool(
            ...
            description="Get vLLM engine performance metrics",
            inputSchema={
                "type": "object",
                "properties": {}
            }
...
async def call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    if name == "analyze_ray_logs":
        ...
        status_msg = f"""
Ray Job Status Overview:
=======================
Status: {summary['job_summary']['status']}
Total Samples: {summary['job_summary']['total_samples']}
Overall Throughput: {summary['job_summary']['overall_throughput']:.2f} samples/second

Resource Allocation:
- CPUs: {summary['resource_status']['cpus_used']}/{summary['resource_status']['cpus_total']}
- GPUs: {summary['resource_status']['gpus_used']}/{summary['resource_status']['gpus_total']}
- Pending CPU Demand: {summary['resource_status']['pending_cpu_demand']}
- Pending GPU Demand: {summary['resource_status']['pending_gpu_demand']}
- Pending Actors: {summary['resource_status']['pending_actors']}
- Pending Tasks: {summary['resource_status']['pending_tasks']}

Errors: {len(summary['job_summary']['errors'])}
Warnings: {len(summary['job_summary']['warnings'])}
"""
        return [TextContent(type="text", text=status_msg)]
    
    elif name == "check_resource_allocation":
        status = analyzer.resource_status
        
        allocation_msg = f"""
Resource Allocation Status:
==========================
CPU Usage: {status.cpus_used}/{status.cpus_total} ({(status.cpus_used/status.cpus_total*100):.1f}% utilized)
GPU Usage: {status.gpus_used}/{status.gpus_total} ({(status.gpus_used/status.gpus_total*100):.1f}% utilized)

Pending Demands:
- CPU: {status.pending_cpu_demand}
- GPU: {status.pending_gpu_demand}
- Actors waiting: {status.pending_actors}
- Tasks waiting: {status.pending_tasks}

{"⚠️  Resource bottleneck detected!" if status.pending_cpu_demand > 0 or status.pending_gpu_demand > 0 else "✓ No pending resource demands"}
"""
        ...
        if not stages:
            return [TextContent(type="text", text="No pipeline stages detected yet.")]
        
        progress_msg = "Pipeline Stage Progress:\n========================\n"
        for stage_name, stage in stages.items():
            progress_bar = "█" * int(stage.progress_percent / 5) + "░" * (20 - int(stage.progress_percent / 5))
            progress_msg += f"\n{stage_name}:\n"
            progress_msg += f"  Progress: [{progress_bar}] {stage.progress_percent:.1f}%\n"
            progress_msg += f"  Items: {stage.processed_items}/{stage.total_items}\n"
            progress_msg += f"  Throughput: {stage.throughput:.2f} {stage.throughput_unit}\n"
        
        return [TextContent(type="text", text=progress_msg)]
    
    elif name == "get_vllm_metrics":
        metrics = analyzer.vllm_metrics
        
        if not metrics:
            return [TextContent(type="text", text="No vLLM metrics detected yet.")]
        
        metrics_msg = "vLLM Engine Metrics:\n====================\n"
        for engine_id, metric in metrics.items():
            metrics_msg += f"\nEngine {engine_id}:\n"
            metrics_msg += f"  Prompt Throughput: {metric.prompt_throughput:.1f} tokens/s\n"
            metrics_msg += f"  Generation Throughput: {metric.generation_throughput:.1f} tokens/s\n"
            metrics_msg += f"  Running Requests: {metric.running_requests}\n"
            metrics_msg += f"  Waiting Requests: {metric.waiting_requests}\n"
            metrics_msg += f"  GPU KV Cache Usage: {metric.gpu_kv_cache_usage:.1f}%\n"
            metrics_msg += f"  Prefix Cache Hit Rate: {metric.prefix_cache_hit_rate:.1f}%\n"
